...
// import { getCartItemCount, cartEvents } from '../utils/cartUtils';
...
import { useNavigate } from 'react-router-dom';
import { ThemeContext } from '../contexts/ThemeContext';
import { getCartItemCount, cartEvents } from '../utils/cartUtils'; // Import cartEvents
import Cart from './Cart';
...
const NavBar = () => {
    ...
    useEffect(() => {
        ...
        // Listen for cart updates from our custom event emitter
        cartEvents.on('cartUpdated', updateCartCount);

        // Also listen for storage events from other tabs (optional)
        window.addEventListener('storage', updateCartCount);

        return () => {
            cartEvents.off('cartUpdated', updateCartCount);
            window.removeEventListener('storage', updateCartCount);
        };
    }, []);
